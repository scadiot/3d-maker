...
class Gizmo:
    ...
    def _update_translate_drag(self, mx, my, scene, camera):
        ...
        if self.drag_start_edge_verts:
            ...
            for i, start_verts in self.drag_start_verts_all.items():
                if i < len(scene.polygons):
                    scene.polygons[i] = [math3d.vadd(v, move) for v in start_verts]
    ...
    def _update_rotate_drag(self, mx, my, scene, camera):
        ...
        for i, start_verts in self.drag_start_verts_all.items():
            if i < len(scene.polygons):
                scene.polygons[i] = [math3d.rotate_point(v, self.drag_center, axis_dir, delta)
    ...
    def _update_scale_drag(self, mx, my, scene, camera):
        ...
        scene.polygons[scene.selected_idx] = math3d.quad_compose(
    ...
    def _scale_handle_positions(self, quad, camera):
        center, wa, ha, hw, hh = math3d.quad_decompose(quad)  # quad à 4 sommets uniquement
